...
let Referals = {
  ...
  getAccountForTrx: 'SELECT m.address,f.quantity,f.insert_time,f.remain_month,f.account_id,m.passphrase,m.publickey FROM migrated_etps_users m INNER JOIN frozen_addition f ON (m.id = f.account_id) order by f.account_id asc',
  ...
  getStakeInfo: 'SELECT * FROM stake_orders WHERE "senderId" = ${address} AND "insertTime" < \'91236250\'',

  updateStakeStatus: 'UPDATE stake_orders SET "status" = \'0\' WHERE "senderId" = ${address} AND "stakeId" = ${stake_id}'
